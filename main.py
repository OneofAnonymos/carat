...
# تنظیم وزن رنک‌ها برای انتخاب تصادفی
RARITY_WEIGHTS = {
    "Common": 60,
    "Rare": 25,
    "Epic": 10,
...
# بارگذاری کاراکترها از فایل
with open("characters.json", "r", encoding="utf-8") as f:
    CHARACTERS = json.load(f)

# وضعیت شخصیت‌های فعال
active_characters = {}
...
# انتخاب تصادفی کاراکتر بر اساس وزن رنک‌ها
def select_random_character():
    ranked_characters = {}
    for char in CHARACTERS:
        rank = char["rank"]
        ranked_characters.setdefault(rank, []).append(char)
...
async def mycollection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = str(update.effective_user.id)
    if user_id not in user_data or not user_data[user_id]["characters"]:
...
async def gift(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 2:
        await update.message.reply_text("❗️ استفاده صحیح: /gift [username] [نام شخصیت]")
        return
